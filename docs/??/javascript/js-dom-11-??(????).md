...
| 说明 | 详情 |
|------|------|
| 获取方式 | 在事件绑定的回调函数的第一个参数就是事件对象 |
| 命名习惯 | 一般命名为 event、ev、e |
...
| 步骤 | 说明 |
|------|------|
| 获取 DOM 元素 | 使用 `querySelector` 或 `querySelectorAll` 获取需要绑定事件的元素 |
| 定义事件处理函数 | 创建处理事件的函数，函数参数为事件对象 |
| 绑定事件监听器 | 使用 `addEventListener` 方法为元素绑定事件和处理函数 |
| 使用事件对象 | 在事件处理函数中通过事件对象访问所需的属性和方法 |
