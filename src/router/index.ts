...
function resolvePage(mode: string) {
  const loader = pageModules[`../pages/${mode}`]
  return (loader || (() => import('../pages/NotFound.vue'))) as RouteRecordRaw['component']
}

// 按配置为每个挂载目录生成路由，绑定对应页面类型（meta.root 供页面识别所在目录）
const docRoutes: RouteRecordRaw[] = siteConfig.docRoots.map(root => ({
