 ...
 */
export async function processAdmonitions(md) {
...
export function processAdmonitionsSync(md) {
  if (!md) return md
  ...
  for (let i = blocks.length - 1; i >= 0; i--) {
    ...
    const innerHtml = marked.parse(block.raw)
 ...
 */
export function addHeadingAnchors(html, sharedIds) {
  if (!html) return ''
